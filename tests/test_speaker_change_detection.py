...
class TestSpeakerChangeDetection:
    ...
    def test_segments_to_text_with_speakers_shows_speaker_only_on_change(
        ...
        segments = [
            ...
            TranscriptionSegment(
                6.0, 9.0, "That's good to hear.", speaker="SPEAKER_01"
            ),
    ...
    @patch("src.transcriber.torch")
    @patch("src.transcriber.whisper")
    def test_segments_to_text_with_timestamps_shows_speaker_only_on_change(
        self, mock_whisper, mock_torch
    ):
    ...
    @patch("src.transcriber.torch")
    @patch("src.transcriber.whisper")
    def test_segments_to_text_with_timestamps_no_speaker_flag(
        self, mock_whisper, mock_torch
    ):
    ...
    @patch("src.transcriber.torch")
    @patch("src.transcriber.whisper")
    def test_segments_to_text_with_speakers_single_speaker(
        self, mock_whisper, mock_torch
    ):
    ...
    @patch("src.transcriber.torch")
    @patch("src.transcriber.whisper")
    def test_segments_to_text_with_speakers_alternating_speakers(
        self, mock_whisper, mock_torch
    ):
