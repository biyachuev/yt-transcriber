...
class TestSummarizer:
    ...
    def test_initialization_ollama(self, mock_get_cache, mock_requests_get):
        ...
        summarizer = Summarizer(
            backend=SummarizeOptions.OLLAMA, model_name="qwen2.5:3b"
        )
    ...
    def test_initialization_openai(self, mock_rate_limiter, mock_get_cache):
        ...
        summarizer = Summarizer(backend=SummarizeOptions.OPENAI_API, model_name="gpt-4")
    ...
    def test_ollama_model_not_found(self, mock_requests_get):
        ...
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"models": [{"name": "different-model:1b"}]}
        mock_requests_get.return_value = mock_response

        with pytest.raises(RuntimeError, match="Model 'qwen2.5:3b' not found"):
            Summarizer(backend=SummarizeOptions.OLLAMA, model_name="qwen2.5:3b")
    ...
    @patch("src.summarizer.get_openai_rate_limiter")
    @patch("openai.OpenAI")
    def test_summarize_with_openai(
        self, mock_openai_class, mock_rate_limiter, mock_get_cache
    ):
    ...
    def test_no_cache_when_disabled(self, mock_get):
        ...
        summarizer = Summarizer(backend=SummarizeOptions.OLLAMA, use_cache=False)
